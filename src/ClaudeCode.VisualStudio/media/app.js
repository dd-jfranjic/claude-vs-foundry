(function () {
  ...
  function effortDesc(id) {
    // Levels map to the CLI's --effort (adaptive thinking depth), not a fixed token budget.
    switch (id) {
      case "low": return "fast, direct answers";
      case "medium": return "balanced reasoning";
      case "high": return "deep reasoning (model default)";
      case "extrahigh": return "xhigh · deepest sustained reasoning";
      case "max": return "unbounded thinking (this session)";
      case "ultracode": return "xhigh + multi-agent workflows";
      default: return "model/CLI default effort";
    }
  }
  ...
  function renderCustomModel() {
    // Third-party host (Foundry/Bedrock/Vertex): the picker carries only the single
    // "Default (deployment)" entry, and the first-party ids MODEL_SUGGESTIONS advertises
    // don't exist there — ask for the provider's deployment name instead of suggesting them.
    const thirdParty = models.length === 1 && models[0] && models[0].id === "default";
    let h = '<h3>Custom model <button class="close-x">×</button></h3>';
    h += '<input type="text" class="palette-input" id="customModel" placeholder="' + (thirdParty ? "Deployment name — Enter to apply, Esc to go back" : "Model id or alias — Enter to apply, Esc to go back") + '" spellcheck="false" />';
    h += '<div class="note err" id="customModelErr"></div>';
    if (thirdParty) {
      h += '<div class="note">Type the DEPLOYMENT name exactly as it appears in your provider portal (e.g. Foundry → Deployments).</div>';
    } else {
      h += '<div class="note">Type any model id or alias, or pick a known one:</div>';
    }
    h += '<div id="customSuggest">';
    if (!thirdParty) MODEL_SUGGESTIONS.forEach((s) => {
