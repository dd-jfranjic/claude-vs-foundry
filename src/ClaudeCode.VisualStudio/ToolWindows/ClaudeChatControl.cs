...
namespace ClaudeCode.VisualStudio
{
    ...
    public class ClaudeChatControl : UserControl
    {
        ...
        private string _effort = "extrahigh";         // --effort xhigh out of the box (deep code analysis)
        ...
        private void SendSetupStatus()
        {
            _ = System.Threading.Tasks.Task.Run(() =>
            {
                try
                {
                    ...
                    // Env-key auth (Microsoft Foundry / Bedrock / API key) never writes a
                    // credentials file — those setups are signed in by construction.
                    bool loggedIn = cliFound && (AccountService.HasStoredToken() || AccountService.HasEnvCredentials());
        ...
        private void SendInit()
        {
            _host.PostMessage("init", new
            {
                ...
                // On a third-party host (Foundry/Bedrock/Vertex) the stock picker ids are
                // first-party model names that DON'T exist there — only the CLI-resolved
                // default (the host's deployment) and Custom (type a deployment name) are safe.
                models = AccountService.UsesThirdPartyProvider()
                ? new object[]
                {
                    new { id = "default", name = "Default (deployment)", desc = "Your provider deployment (resolved by the CLI/env) · use Custom for another deployment name", ratio = 5.0 },
                }
                : new object[]
                {
