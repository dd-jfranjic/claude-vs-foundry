...
namespace ClaudeCode.VisualStudio.Services
{
    ...
    public static class AccountService
    {
        ...
        public static async Task<AccountData> FetchAsync()
        {
            ...
            try
            {
                ...
                if (token == null)
                {
                    ...
                    {
                        // Env-key auth (Foundry/Bedrock/API key) has no OAuth account to query;
                        // per-session cost and tokens still stream into the chat itself.
                        result.Error = "Account usage isn't available with environment-key auth (Foundry/Bedrock/API key). Per-session cost and tokens still appear in the chat.";
                        return result;
                    }
        ...
        /// <summary>
        /// True when the CLI authenticates from environment variables — Microsoft Foundry,
        /// Amazon Bedrock, Vertex, or a direct API key — instead of a stored `claude login`.
        /// No credentials file ever exists on these setups, so "log in" guidance would be wrong.
        /// </summary>
        public static bool HasEnvCredentials()
        {
            string[] names =
            {
                "ANTHROPIC_FOUNDRY_API_KEY", "ANTHROPIC_FOUNDRY_AUTH_TOKEN", "CLAUDE_CODE_USE_FOUNDRY",
        ...
        /// <summary>
        /// True when requests go through a third-party host (Microsoft Foundry / Bedrock /
