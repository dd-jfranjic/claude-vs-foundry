...
namespace ClaudeCode.VisualStudio.Services
{
    ...
    public sealed class ClaudeSession : IDisposable
    {
        ...
        private string BuildArguments(ClaudeCliLocator.Result cli)
        {
            ...
            var pickerDefault = string.IsNullOrEmpty(_options.Model) ||
                                string.Equals(_options.Model, "default", StringComparison.OrdinalIgnoreCase);
            if (pickerDefault && AccountService.UsesThirdPartyProvider())
            {
                // Third-party hosts (Microsoft Foundry / Bedrock / Vertex) know DEPLOYMENT
                // names, not the first-party id DefaultModel carries — omit --model so the CLI
                // resolves the session model from its own configuration (ANTHROPIC_MODEL / pins).
            }
            else
            {
                sb.Append(" --model ").Append(pickerDefault ? DefaultModel : _options.Model);
            }
            var effortFlag = EffortFlagFor(_options.Effort);
            if (effortFlag != null)
            {
                // `--effort` (adaptive-thinking depth) replaces the legacy --max-thinking-tokens
                // budget: Opus 4.8-class models reject a manual thinking budget, and the CLI
                // calibrates each named level per model.
                sb.Append(" --effort ").Append(effortFlag);
            }
        ...
        /// <summary>Maps the composer's effort choice to the CLI's `--effort` level name.
        /// null (composer "Off") omits the flag so the CLI/env default rules — e.g. a
        /// fleet-wide CLAUDE_CODE_EFFORT_LEVEL, which always takes precedence anyway.</summary>
        internal static string EffortFlagFor(string effort)
        {
            switch ((effort ?? "").ToLowerInvariant())
            {
                ...
                default: return null; // "none"/null -> model/CLI default
            }
        }
